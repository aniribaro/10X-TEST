...
// Launch the app with availability check
async function launchApp(event) {
  ...
  if (isRunning) {
    ...
    showAppNotRunningModal();
  }
}

// Show modal when app is not running
function showAppNotRunningModal() {
  // Check if modal already exists
  let modal = document.getElementById('app-not-running-modal');

  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'app-not-running-modal';
    modal.className = 'modal';
    modal.innerHTML = `
      <div class="modal-backdrop" onclick="closeAppModal()"></div>
      <div class="modal-content">
        ...
        <h3>App Not Running</h3>
        <p>The 10X Accountability Coach app is not currently running on localhost:3000.</p>
        <div class="modal-instructions">
          <h4>How to start the app:</h4>
          <div class="code-block small">
            <code>cd ui && npm run dev</code>
          </div>
          <p class="hint">Or use Claude Code: <code>claude "start my app"</code></p>
        </div>
        <div class="modal-actions">
          <button class="btn btn-ghost" onclick="closeAppModal()">Close</button>
          <button class="btn btn-primary" onclick="checkAppAgain()">
            <span>Try Again</span>
          </button>
        </div>
      </div>
    ...
    style.textContent = `
      ...
      .modal .modal-instructions h4 {
        font-size: 14px;
        font-weight: 600;
        color: #fff;
        margin-bottom: 12px;
      }
      .modal .modal-instructions .code-block {
        background: rgba(0, 0, 0, 0.3);
        border-radius: 8px;
        padding: 12px 16px;
        margin-bottom: 12px;
      }
      .modal .modal-instructions .code-block code {
        color: #22c55e;
        font-family: monospace;
        font-size: 14px;
      }
      .modal .modal-instructions .hint {
        font-size: 13px;
        color: #71717a;
        margin: 0;
      }
      .modal .modal-instructions .hint code {
        color: #8b5cf6;
...
// Close the app not running modal
function closeAppModal() {
  const modal = document.getElementById('app-not-running-modal');
  if (modal) {
...
// Check app again and retry
async function checkAppAgain() {
  ...
  if (isRunning) {
    ...
    showToast('App still not running. Please start the server first.');
  }
}
